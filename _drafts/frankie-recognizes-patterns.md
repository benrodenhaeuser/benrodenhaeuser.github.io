...
description: 'Defining parametrized routes: part 03 of the "Understanding Sinatra" series.'
date: 2018-06-03
---
...
In the [previous installation][2] of this blog post series, Frankie picked up a bit of Sinatra’s signature DSL magic. Next, let’s expand Frankie’s capabilities a bit. What is sorely missing from our toy version of Sinatra so far is the ability to *parametrize routes*:
...
In this example, `'greatest-hits'` and `'my-way` are essentially arguments that fill in the slots provided by the parameters `:album` and `:song`. As the route block shows, the mapping from parameters to arguments supplied should be available as key-value pairs within a `params` hash, for Frankie users to freely make use of.

Here is an idea how to make this work: Let’s match the request path against a regular expression stored along with the route in `Frankie::Application`. In our example, we expect the matching to produce the captures `greatest-hits` and `my-way`. These should be linked with the route parameters to form a hash
...
Remember that `@request` is an instance of `Rack::Request`. Conveniently, `@request` is a hash we can use for our purposes.

Next, let’s adapt our mechanism for *storing* routes with parameters. Keeping in mind what was said in [part 01 of this series][3], this needs to happen at the *class level*:
...
The `route` method compiles a given path (possible containing parameters) into a `pattern` (a regular expression) and a set of string `keys`. During request handling, those strings will eventually become keys in the `params` hash.
...
> Sinatra goes out of its way to allow users flexibility in making use of route parameters. For instance, regular expressions are allowed as route paths, and route paths may contain wildcards („splats“) and/or optional parameters. So we have merely scratched the surface here. Also, the way Sinatra stores and processes parametrized routes has changed somewhat with the advent of the s[Mustermann string processing library][4] in Sinatra 2.0. Our approach here is closer to how things used to work up to Sinatra 1.4.x.
{: .aside}
...
[2]:	/2018/06/02/frankie-goes-top-level/
