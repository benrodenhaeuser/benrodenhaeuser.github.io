...
Note that we have replaced the invocation of `sort_by` with calls to `map`, `sort` and `<=>`. This is obviously a lot more cumbersome than using `sort_by` itself, but it paves the way towards our reimplementation.

Generalizing the above approach to our example, here is an implementation of a method `Enumerable#my_sort_by`:
...
Both `sort` and `sort_by` are based on comparisons. Comparison-based sorting [has a lower bound](https://www.cs.cmu.edu/~avrim/451f11/lectures/lect0913.pdf) of $n \log n$ in the worst case, which is to say that it is not possible to come up with an algorithm for sorting that performs better in a worst-case scenario. The reason is that, in the worst case, $n \log n$ comparisons have to be made in order to arrive at a sorted array. Internally, Ruby [uses quicksort for sorting](https://www.igvita.com/2009/03/26/ruby-algorithms-sorting-trie-heaps/), an algorithm that has a worst case complexity of $n^2$, but is $n \log n$ on average (which is the lower bound for the average case as well).

Since quicksort is the sort algorithm powering both `sort` and `sort_by`, wouldn't it be reasonable to think that both methods should have the same performance? The answer is no. The reason is that the picture is complicated by the fact that we often do not wish to sort a given collection *as is* (i.e., relying on the comparison operator the elements of the collection come with), but rather relying on some specific property of these objects, i.e., a sort criterion, or *sort key*. We may want to sort user entries based on the user's last name, perhaps game moves based on their expected utility – or strings based on their integer value, as in our running example above.

Now unless the values we want to sort already come *equipped* with a sort key, we will have to generate the keys ourselves. This takes time. And this is where `sort` and `sort_by` differ.

Both of these methods allow us to do key-based sorting by passing a block with the method invocation. Suppose we have a method `key` that transforms the elements of `list` to sort keys of the required kind. Then we can sort our list with the `sort` method as follows:  
...
It looks like using `sort_by` saves a little bit of typing. But we are also saving a lot of computation time, potentially. Remember: what is happening "under the hood", when we invoke `sort_by` in the way just described, is something like this:
